...
if (empty($_SESSION['questions'])) {
    ...
    $max_limit = $_SESSION['subtraction_max_limit'] ?? 1000; // Default to 1000 if not set

    for ($i = 0; $i < $num_questions_to_ask; $i++) {
        $nombre1 = rand(6000, $max_limit); // Use the limit here, adjusted range for subtraction
        $nombre2 = rand(100, 6000); // Adjusted range for subtraction, nombre2 <= nombre1 generally
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['mot'])) { // Changed to accept array of 'mot'
    ...
    $operationType = 'subtraction'; // Type d'opération fixe
    ...
    log_adresse_ip("logs/log.txt", "correction_subtraction.php - ".$_SESSION['prenom']." - Correction Quiz Complet Soustraction");
    ...
    echo '<input type="submit" value="Rejouer le Quiz de Soustractions">';
    echo '</form>';
...
<head>
    <meta charset="utf-8">
    <title>Question Soustractions - Toutes les questions</title>
</head>
        ...
        <table border="0" cellpadding="0" cellspacing="0">
            <tr>
                <td style="width:1000px;height:430px;background-image:url('./images/NO.jpg');background-repeat:no-repeat;">
                    <center>
                        <h1>Quiz de Soustractions</h1><br />
                        <?php if (!empty($_SESSION['questions']) && $_SESSION['question_number'] <= $num_questions_to_ask): ?>
                            <form action="./question.php" method="post">
                                ...
                                <?php endforeach; ?>
                                <input type="submit" value="Correction du Quiz de Soustractions">
                            </form>
                        <?php endif; ?>
                    </center>
                </td>
