...
if (empty($_SESSION['questions'])) {
    ...
    $max_limit = $_SESSION['addition_max_limit'] ?? 1000; // Default to 1000 if not set

    for ($i = 0; $i < $num_questions_to_ask; $i++) {
        $nombre1 = rand(1, $max_limit); // Use the limit here
        $nombre2 = rand(1, $max_limit); // Use the limit here
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['mot'])) { // Changed to accept array of 'mot'
    ...
    $operationType = 'addition'; // Type d'opération fixe
    ...
    log_adresse_ip("logs/log.txt", "correction.php - ".$_SESSION['prenom']." - Correction Quiz Complet");
    ...
    echo '<input type="submit" value="Rejouer le Quiz">';
    echo '</form>';
...
<head>
    <meta charset="utf-8">
    <title>Question Additions - Toutes les questions</title>
</head>
                    ...
                    <center>
                        ...
                        <?php if (!empty($_SESSION['questions']) && $_SESSION['question_number'] <= $num_questions_to_ask): ?>
                            <form action="./question.php" method="post">
                                ...
                                <?php endforeach; ?>
                                <input type="submit" value="Correction du Quiz">
                            </form>
                        <?php endif; ?>
                    </center>
                </td>
